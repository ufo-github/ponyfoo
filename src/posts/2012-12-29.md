...
I'll probably try and support **IE 10**, but that's as far as I'm willing to go. Things like `localStorage` and `history` are supported by _IE8+_ and _IE10+_ respectively, and I didn't want to spend my time patching behavior in earlier versions of _IE_ just yet.
...
It's not like I'm going to get to [20 million monthly page views](http://theoatmeal.com/) anytime soon, but that's **no excuse** for not improving the experience of the few adventuring ones that _dare navigate_ my site.
...
I explained all of those properties, except for the _alias_. The alias would be the _route_ that we register with the HTML 5 [history](https://developer.mozilla.org/en-US/docs/DOM/Manipulating_the_browser_history) API.
...
We didn't change anything else, thus `!soft` will always be _truthy_, because `!undefined` is _truthy_. And each template activation will change the _navigation history_ with the _route sugar_ defined in `template.alias`.
...
In order to **preserve navigation**, since we're manipulating `history` with `history.pushState` we need to handle the `popstate` event, this encompasses two steps. The first is to handle `popstate`, so we'll go ahead and do that:
	...
	$(function(){
		...
		$(window).trigger("popstate"); // manual trigger fixes an inconsistency with Firefox.
	});

Here we simply take the key from data passed to the `state`, or from the `document.location`, and pass that along to `activate`, with `soft` set to true, meaning we won't be _pushing_ a new `state` this time.

The manual trigger fixes an [inconsistency with Firefox](http://hacks.mozilla.org/2011/03/history-api-changes-in-firefox-4/) where it wouldn't `popstate` on document load.
